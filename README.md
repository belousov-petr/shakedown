...
I was running a 17-agent intelligence pipeline and every time I wanted to review the whole thing, I'd write a long prompt from scratch. Every time, I'd forget something. Sometimes security. Sometimes backup validation. Sometimes I just wouldn't look at how the agents coordinate with each other.
...
Checks whether backups actually restore (not just whether they exist). Traces what happens when components fail. See `references/resilience-testing.md` for the hands-on test recipes.

### Phase 7: Follow-up handlers
After the report is rendered (and the file is written if File mode was chosen), the skill surfaces a follow-up menu and waits for you to pick one of:

- **Fix them all** - implement every Top 10 recommendation in priority order
- **Fix selected** - "fix #2, #5, #7" or "fix all Critical/High"
- **Re-run Phase 5** - refresh security / readiness / recommendations only
- **Compare with last** - diff this report against the previous saved one
- **Create GitHub issues** - one issue per recommendation, via `gh`
- **Done** - exit, no further action

The handlers run in the same session, with the same context. No separate command, no re-discovery.
...
| Mode | What happens |
|---|---|
| **Inline mode** (default) | Full report in the session. Nothing saved to disk. |
| **File mode** | Full report in the session AND saved to `shakedown-YYYY-MM-DD.md` in the project root. Pick this when you want a standalone file to archive, share, or feed into a fresh session later. |
...
4. Architecture & Code Quality - MECE gaps, contradictions, algorithm efficiency
5. Error Handling, Resilience & Failure Modes - crash paths, silent failures, edge cases, retry patterns
6. Performance & Bottleneck analysis - timing, parallelism, scaling, cost
7. Code and Storage Efficiency - empty files, duplicates, dead deps, bloat
8. Skill Standards compliance (if applicable) - spec, description, instructions, evals
9. Security & Data Exposure - secrets, injection risks, PII, licensing
10. Logging & Observability gaps
11. Documentation Quality - accuracy, completeness, onboarding
12. Goal Fulfillment - stated vs actual behavior
13. Blind Spots nobody is watching
14. Objective Clarity ratings (8 dimensions, scored 1-10)
15. Overall score with justification
16. Production Readiness (10 gates; you call the ship/no-ship yourself)
17. Top 10 Ranked Recommendations with effort estimates
18. Value Assessment - problem clarity, audience, maturity, differentiation
19. The Uncomfortable Question

(For Phase 3 - Data Store Diagnostics - and Phase 6 - Resilience Testing - see "How it works" above; both run only when applicable.)
...
| Traditional security (always applies) | `references/security-traditional.md` |
| LLM / GenAI / MCP risks (if you use them) | `references/security-llm.md` |
| Agentic risks - tool misuse, goal hijack, multi-agent (if you use agents) | `references/security-agentic.md` |
| AI governance & red teaming (compliance contexts) | `references/security-governance.md` |
...
```
shakedown/
├── SKILL.md                              # The skill - 7-phase orchestrator
├── references/
...
│   ├── performance-analysis.md           # Section 4.6: timing, scaling, cost (with cProfile / pg_size / cost recipes)
...
│   ├── security-traditional.md           # Section 5.1A: secrets, injection, PII, supply chain, licensing (always loaded)
│   ├── security-llm.md                   # Section 5.1B/D/E: OWASP LLM Top 10, MCP, GenAI Data (LLM projects)
│   ├── security-agentic.md               # Section 5.1C/H: OWASP Agentic Top 10 + 23 concrete test procedures (agent projects)
│   ├── security-governance.md            # Section 5.1F/G: AI Governance, EU AI Act, NIST RMF, Red Teaming (compliance contexts)
...
│   ├── resilience-testing.md             # Phase 6: backup and resilience tests (pg_restore / sqlite / mongo recipes)
...
│   └── parity-check.py                   # Detects feature drift between README.md and SKILL.md
├── README.md
└── LICENSE                               # MIT
```

The main SKILL.md is a clean orchestrator - phases, flow, output templates. All detailed checklists live in `references/` (14 files) and are loaded on demand. The four `security-*.md` files are loaded conditionally based on project type (LLM / agentic / governance triggers) so non-LLM projects don't pay the token cost. Each reference file includes scope boundary notes to prevent overlap.
